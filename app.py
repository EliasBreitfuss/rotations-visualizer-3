...
winkelgeschwindigkeit = 2 * np.pi * u_sek  # rad/s
...
# --- Frames für Animation vorbereiten ---
frames = []
num_frames = 100
t_values = np.linspace(0, 1, num_frames)  # normierte Zeit, Dauer = 1 Sekunde pro Loop

for t in t_values:
    x_point = (durchmesser/2) * np.cos(winkelgeschwindigkeit * t)
    y_point = (durchmesser/2) * np.sin(winkelgeschwindigkeit * t)
    frames.append(go.Frame(data=[
        go.Scatter(x=x_circle, y=y_circle, mode='lines', line=dict(color='blue')),
